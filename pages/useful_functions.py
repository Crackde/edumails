...
def random_four_digit_PIN():
    dato=random.choice(range(1000, 10000))
    print(dato,' este es el pin')
    return dato
...
def username_generator(stringlength):
    ...
    username=''.join(secrets.choice(lettersAndDigits) for i in range(stringlength)).lower()
    print(username,' este es el ususario')
    return username
...
def password_Generator(stringlength):
    ...
    passw=''.join(secrets.choice(letter_digit_symbol) for i in range(stringlength))
    print(passw,' esta es la pass')
    return passw
